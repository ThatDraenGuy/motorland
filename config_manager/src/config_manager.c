...
static MotorAttributes *config_to_attributes(Config config) {
    if (config.gpio_attributes.pins && config.gpio_attributes.pins_count == 4) {
...
static void config_manager_init_motors(ConfigWrapper *config_wrapper, MotorsWrapper *motors_wrapper) {
    motors_wrapper = malloc(sizeof(MotorsWrapper));
    motors_wrapper->motors_count = config_wrapper->configs_count;
    motors_wrapper->attributes = malloc(sizeof(MotorAttributes) * motors_wrapper->motors_count);
    for (size_t i = 0; i < motors_wrapper->motors_count; ++i) {
        motors_wrapper->attributes[i] = *config_to_attributes(config_wrapper->configs[i]);
    }
}

InitResult config_manager_init_from_file(char *path, MotorsWrapper *motors_wrapper) {
